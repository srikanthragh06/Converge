...
- On a tool that takes a pagination \`cursor\` argument, never invent one on a first call. Only pass \`cursor\` when you are handing back a \`nextCursor\` value a previous call to that same tool actually returned to you in this conversation. A made-up cursor (for example \`{ id: 1 }\` or \`{ lastVisitedAt: null }\`) does not error — it silently returns zero results, even when matching items exist, and you have then wrongly concluded nothing exists.
