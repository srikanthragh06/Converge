...
// Debounces the socket emit after the last keystroke so network traffic is minimal.
...
export default function DocumentTitle({
    ...
    // How long after the last keystroke before the socket emit fires (ms).
    ...
    // True from the first keystroke until the debounce fires — drives the blur effect.
    const [isSaving, setIsSaving] = useState(false);
    // Debounce timer ref — reset on every keystroke, fires emit after DEBOUNCE_MS of silence.
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        debounceRef.current = setTimeout(() => {
            socket.emit("update_doc_title", next);
            setIsSaving(false);
        }, DEBOUNCE_MS);
    };
